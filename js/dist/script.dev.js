...
var entradaAdulto = new Entrada('AD', 20, 20000, 15000, 'Entrada Adulto');
var entradaNino = new Entrada('NI', 20, 15000, 13000, 'Entrada Niño');
...
function menu(opcion) {
  while (opcion != 6) {
    ...
    switch (opcion) {
      ...
      case '3':
        /*Consulta Registro de Compras Adultos */
        verRegistroCompras('AD');
        break;

      case '4':
        /*Consulta Registro de Compras Niños*/
...
function verRegistroCompras(indicador) {
  if (indicador == 'AD') {
    if (arrayComprasA.length == 0) {
      ...
      for (var i = 1; i <= arrayComprasA.length; i++) {
  ...
  if (indicador == 'NI') {
    if (arrayComprasN.length == 0) {
      ...
      for (var _i = 1; _i <= arrayComprasA.length; _i++) {
        alert('Compra ' + _i + ' por: $' + arrayComprasN[_i - 1]);
      }
    }
  }
}
